...
class NgaScreenshotter:
    ...
    async def _capture_context_cookies(self, context, has_user_cookie: bool) -> None:
        if not self._auto_refresh_cookies or not has_user_cookie:
            return
        if self._cookie_storage_path is None:
            return
        now = time.monotonic()
